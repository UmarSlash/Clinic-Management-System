{{-- <div>
    <div class="{{ !$showModel ? 'hidden' : 'block' }} fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center"
        wire:key="doctor-{{ $showModel }}">
        <div class="relative p-4 w-full max-w-lg max-h-full">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                <!-- Modal header -->
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ $doctor->exists ? 'Edit Doctor' : 'Add Doctor' }}
                    </h3>
                ...
                <form class="p-4 md:p-5">
                    <div class="grid gap-4 mb-4 grid-cols-1">
                        ...
                        <div>
                            <div class="relative">
                                <input wire:model="specialty" type="text" id="specialty" aria-describedby="specialty_help"
                                    class="block px-4 pb-4 pt-6 w-full text-md text-gray-900 bg-transparent rounded-lg border-1 appearance-none dark:text-white peer
                                    dark:border-gray-500 border-gray-600 dark:focus:border-gray-500 focus:outline-none focus:ring-0 focus:border-gray-600"
                                    placeholder=" " autofocus />
                                <label for="specialty"
    ...
    <div class="container mx-auto p-4">
        ...
        <div class="bg-white dark:bg-gray-700 shadow rounded-lg p-6">
            ...
            <form>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <input type="text" wire:model="gender" placeholder="Gender" class="border p-2 rounded w-full">
                        @error('gender')
                            <span class="text-sm text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                    <div>
                        <input type="text" wire:model="phone_no" placeholder="Phone No" class="border p-2 rounded w-full">
                        @error('phone_no')
                            <span class="text-sm text-red-600">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <input type="text" wire:model="specialty" placeholder="Specialty" class="border p-2 rounded w-full">
                    @error('specialty')
                        <span class="text-sm text-red-600">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <button wire:click="save" type="button"
                    ...
                    <span>{{ $doctor->exists ? __('Update') : __('Add') }}</span>
                </button>
            </form>
        </div>
    </div>
